...
%--------------------------------------------------------------------------
    ...
    methods (Access = private)
        ...
        function DecreaseKey(this,i,key)
            ...
            elseif (key.dist > this.x(i).dist)
                % Decrease key error
                MinHeap.DecreaseKeyError();
            end
            this.x(i).dist = key.dist;
            this.x(i).name = key.name;
            this.x(i).start = key.start;
            this.x(i).stop = key.stop;
            
            while ((i > 1) && (this.x(Heap.parent(i)).dist > this.x(i).dist))
        ...
        function MinHeapify(this,i,size)
            ...
            ll = Heap.left(i);
            rr = Heap.right(i);
            if ((ll <= size) && (this.x(ll).dist < this.x(i).dist))
                smallest = ll;
            else
                smallest = i;
            end
            if ((rr <= size) && (this.x(rr).dist < this.x(smallest).dist))
                smallest = rr;
            end
            if (smallest ~= i)
